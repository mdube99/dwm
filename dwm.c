...
static void adatesystrayicongeom(Client *i, int w, int h);
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	ret = x = m->ww - barpadpx - w;
        x = m->ww - barpadpx - w - getsystraywidth();

	drw_setscheme(drw, scheme[LENGTH(colors)]);
	drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
	drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
	drw_rect(drw, x, barpadpx, w, bh, 1, 1);
	x++;
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x, barpadpx, w, bh, 0, text, 0);
			...
			while (text[++i] != '^') {
				if (text[i] == 'c') {
					...
					int c = atoi(text + ++i);
					drw_clr_create(drw, &drw->scheme[ColFg], xcolor[c]);
				} else if (text[i] == 'B') {
					int c = atoi(text + ++i);
					drw_clr_create(drw, &drw->scheme[ColBg], xcolor[c]);
				} else if (text[i] == 'd') {
					drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
					drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
				} else if (text[i] == 'r') {
					...
					drw_rect(drw, rx + x, ry + barpadpx, rw, rh, 1, 0);
				} else if (text[i] == 'f') {
					x += atoi(text + ++i);
				}
			}
...
drawbar(Monitor *m)
{
	int x, y = barpadpx, w, wdelta, sw = 0, stw = 0;
	int th = bh - barpadpx * 2;
	int mw = m->ww - barpadpx * 2;
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (ulall || m->tagset[m->seltags] & 1 << i) /* if there are conflicts, just move these lines directly underneath both 'drw_setscheme' and 'drw_text' :) */
                	drw_rect(drw, x + ulpad, th - ulstroke - ulvoffset, w - (ulpad * 2), ulstroke, 1, 0);
		if (occ & 1 << i)
...
setup(void)
{
	...
	for (i = 0; i < LENGTH(colors); i++)
        ...
        drw_clr_create(drw, &clrborder, bgr);
	/* init system tray */
	updatesystray();
	/* init bars */
	updatebars();
...
updatestatus(void)
{
	if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext)))
		strcpy(stext, " Loading statusbar... ");
	drawbar(selmon);
	updatesystray();
}
